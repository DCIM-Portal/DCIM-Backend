class System < ApplicationRecord
  ...
  class << self
    ...
    attribute_to_fact_map = {
      cpu_model: ['processor0', 'cpu_model'],
# XXX: Next line appears to be impossible. Remove from model?
#      cpu_cores: [],
      cpu_threads: ['num_cpus', 'processorcount'],
...
#      disk_count: [],
    ...
    @@sync_attribute_names.each do |attribute_name|
      if attribute_to_fact_map[attribute_name.to_sym].is_a? Array
  ...
  def refresh!
    ...
    @@sync_attribute_names.each do |attribute_name|
      ...
      self.send("#{attribute_name}=", attribute_value) unless attribute_value.nil?
    end
  ...
  def determine_attribute_value(attribute_name, facts)
    ...
    return self.send("determine_#{attribute_name}", facts) if self.respond_to? "determine_#{attribute_name}"
    nil
  end

  def determine_ram_total(facts)
    ram_total_mb = nil
    ['mem_total', 'memorysize_mb'].each do |fact_name|
      ram_total_mb ||= facts[fact_name]
    end
    return nil if !ram_total_mb
