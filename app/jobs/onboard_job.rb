...
class OnboardJob < ApplicationJob
  ...
  def perform(**kwargs)
    ...
    ActiveSupport::Dependencies.interlock.permit_concurrent_loads do
      promises.each do |bmc_host_id, promise|
        promise.execute
      end
    end

    pool.shutdown
    pool.wait_for_termination(timeout = 1200)
  ...
  def onboard(bmc_host)
    if serial_onboarded?(bmc_host.serial)
      ...
      steps.each do |step|
        ...
        begin
          keep_trying(every: step[:every], timeout: step[:timeout], until: step[:until]) { self.send("try_#{step[:name]}", bmc_host) }
