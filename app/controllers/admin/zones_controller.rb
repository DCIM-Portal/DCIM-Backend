class Admin::ZonesController < AdminController
  ...
  def foreman_add
    ...
    params[:zone].each do |foreman_params|
      result = add_location(foreman_params['name']) unless locations_get.any? { |x| x['name'] == foreman_params['name'] }
  ...
  def create
    ...
    respond_to do |format|
      ...
      if !@logger.error?
        if @zone.save
          result = add_location(params[:zone][:name]) unless locations_get.any? { |x| x['id'] == params[:zone][:id] }
          new_foreman_location_id = JSON.parse(result.body)['id']
  ...
  def foreman_locations
    @foreman_locations = locations_get.map { |h| h.values_at('id', 'name') }
  end
