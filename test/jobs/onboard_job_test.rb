...
class OnboardJobTest < ActiveJob::TestCase
  ...
  test 'store_dependencies defaults to system-wide ForemanApi' do
    ...
    assert_equal Dcim::ForemanApiFactory.instance,
                 onboard_job.instance_variable_get(:@foreman_resource),
                 'Foreman resource dependency not obtained from ForemanApiFactory'
  end
  ...
  test 'keep trying x block flunks timeout' do
    ...
    assert_raises(Timeout::Error) { @onboard_job.keep_trying(every: 1, until: true, timeout: timeout, &-> {}) }
  end
  ...
  test 'store stack trace on job failure' do
    ...
    rescue Dcim::UnsupportedApiResponseError => e
  ...
  test 'onboard not already onboarded' do
    ...
    %i[shutdown power_off pxe discover manage bmc_creds].each do |step|
